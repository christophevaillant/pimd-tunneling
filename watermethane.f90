...
double precision::               beta0(waterdof, methanedof), A0(waterdof, methanedof), A1(waterdof, methanedof), AM(waterdof, methanedof)
...
!-------------------------------------------------------------------------------------------
subroutine clath_grad(x, grad, nwater)
implicit none

integer::              i,j,k, nwater
double precision::     x(nwater*24+3*methanedof), grad(nwater*24+3*methanedof), ewater
double precision::     xwatermeth(51), tempgrad(51), xwater(nwater*9), gradwater(nwater*9)
logical::             gradt
...
do i=1, nwater
   ...
   xwatermeth(25:51)= x(nwater*(waterdof+1)*3 + 1: nwater*(waterdof+1)*3 + methanedof*3)/autoang
   call wmrb_grad(xwatermeth, tempgrad)
   do j=1, waterdof+1
      do k=1, 3
         grad(3*((waterdof+1)*(i-1) + j -1) +k)= grad(3*((waterdof+1)*(i-1) + j -1) +k)&
              +tempgrad(3*(j-1) + k)*autokcal/autoang
...
return
end subroutine clath_grad
!-------------------------------------------------------------------------------------------
subroutine wmrb_grad(x, grad)
!function that calculates the gradient from the water-methane interaction
!for rigid body
...
eint= exp(-beta0(a,b)*r)*(A0(a,b) + A1(a,b)*r + AM(a,b)/r)
...
grad= -beta0(a,b)*exp(-beta0(a,b)*r)*(A0(a,b) + A1(a,b)*r + AM(a,b)/r)
grad= grad + exp(-beta0(a,b)*r)*(A1(a,b) - AM(a,b)/r**2)
